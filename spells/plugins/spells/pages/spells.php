...
if($reload && admin()) {
	BaseSpells::reloadSpells(true);
}

if(admin()) {
	echo $twig->render('spells/views/reload.html.twig');
}
...
$order = 'name';
$spells = [];
$spells_db = SpellModel::where('hide', '!=', 1)->where('type', '<', 4)->orderBy($order)->get();
...
$showColumns = [
	'group' => SpellModel::where('group', '!=', 0)->count() > 0,
	'level' => SpellModel::where('level', '>', 0)->count() > 0,
];

$twig->display('spells/views/spells.html.twig', array(
